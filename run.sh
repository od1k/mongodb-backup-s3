...
echo "=> Backup started"
if mongodump --host ${MONGODB_HOST} --port ${MONGODB_PORT} ${USER_STR}${PASS_STR}${DB_STR} --archive=\${BACKUP_NAME} --gzip ${EXTRA_OPTS} && aws ${REGION_STR} s3 cp \${BACKUP_NAME} \${S3BACKUP} && aws ${REGION_STR} s3 cp \${S3BACKUP} \${S3LATEST} && rm \${BACKUP_NAME} ;then
...
if aws ${REGION_STR} s3 cp \${S3RESTORE} \${RESTORE_ME} && mongorestore --host ${MONGODB_HOST} --port ${MONGODB_PORT} ${USER_STR}${PASS_STR}${DB_STR} --drop --archive=\${RESTORE_ME} --gzip && rm \${RESTORE_ME}; then
